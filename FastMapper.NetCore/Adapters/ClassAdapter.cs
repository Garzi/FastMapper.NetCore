...
namespace FastMapper.NetCore.Adapters
{
        ...
        where TDestination : class
    {
        ...
        private static readonly FastObjectFactory.CreateObject _destinationFactory =
            FastObjectFactory.CreateObjectFactory<TDestination>();
            ...
            Dictionary<int, int> parameterIndexs)
        {
            ...
            if (hasMaxDepth)
            {
                ...
                if (parameterIndexs.ContainsKey(hashCode))
                {
                    ...
                    if (index >= config.MaxDepth) return null;
                }
                else
                {
            ...
            if (properties != null)
                for (var j = 0; j < properties.Length; j++)
                {
                        ...
                        propertyName.StartsWith(property.Name))
                    {
                        ...
                        GetDeepFlattening(property.PropertyType, propertyName.Substring(property.Name.Length),
                            invokers);
                    }
