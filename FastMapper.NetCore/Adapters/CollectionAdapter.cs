        ...
        private static CollectionAdapterModel CreateCollectionAdapterModel()
        {
            ...
            else if (ReflectionUtils.IsCollection(destinationElementType))
            {
                var methodInfo = typeof(CollectionAdapter<,,>)
                    .MakeGenericType(sourceElementType, ReflectionUtils.ExtractElementType(destinationElementType),
                        destinationElementType).GetMethod("Adapt",
                        new[] {sourceElementType, typeof(Dictionary<,>).MakeGenericType(typeof(int), typeof(int))});
                cam.AdaptInvoker = FastInvoker.GetMethodInvoker(methodInfo);
            }
            else
            {
                var methodInfo = typeof(ClassAdapter<,>).MakeGenericType(sourceElementType, destinationElementType)
                    .GetMethod("Adapt",
                        new[] {sourceElementType, typeof(Dictionary<,>).MakeGenericType(typeof(int), typeof(int))});
                cam.AdaptInvoker = FastInvoker.GetMethodInvoker(methodInfo);
            }
        ...
        public static object Adapt(TSource source, object destination, Dictionary<int, int> parameterIndexs)
        {
            ...
            if (hasMaxDepth)
            {
                ...
                if (parameterIndexs.ContainsKey(hashCode))
                {
                    ...
                    if (parameterIndexs[hashCode] >= config.MaxDepth) return null;
                }
                else
                {
