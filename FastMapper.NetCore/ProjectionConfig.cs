...
using FastMapper.NetCore.Models;
...
namespace FastMapper.NetCore
{
    ...
    internal class ProjectionConfig<TSource, TDestination>
    {
        ...
        {
            var key = string.Concat(typeof(TSource).FullName, typeof(TDestination).FullName);
        ...
        public ProjectionConfig<TSource, TDestination> IgnoreMember(
            params Expression<Func<TDestination, object>>[] members)
        {
            if (members != null && members.Length > 0)
            {
                ...
                for (var i = 0; i < members.Length; i++)
                {
                    ...
                    if (memberExp != null) ignoreMembers.Add(memberExp.Member.Name);
                }
        ...
        public ProjectionConfig<TSource, TDestination> IgnoreMember(params string[] members)
        {
            if (members != null && members.Length > 0)
            {
                members = typeof(TDestination).GetProperties().Where(p => members.Contains(p.Name)).Select(p => p.Name)
                    .ToArray();

                if (members != null && members.Length > 0) SetCache(members);
            }

            return this;
        }

        public ProjectionConfig<TSource, TDestination> MapFrom<TKey>(
            Expression<Func<TDestination, TKey>> destinationMember, Expression<Func<TSource, TKey>> sourceExpression)
        {
            if (sourceExpression != null)
            {
                var memberExp = destinationMember.Body as MemberExpression;
                if (memberExp != null)
                    SetCache(new ExpressionModel
                    {
